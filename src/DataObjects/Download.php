...
class Download extends DataObject
{
    ...
    public function PreparedTitle(): string
    {
        return html_entity_decode(str_replace("|", "&shy;", $this->Title));
    }

    public function PublishFiles($CanViewType): void
    {
    ...
    public function PublishFile($file, $CanViewType): void
    {
        $writefile = false;
    ...
    public function ProtectFiles($CanViewType, $ViewerGroups): void
    {
    ...
    public function ProtectFile($file, $CanViewType, $ViewerGroups = null): void
    {
        $writefile = false;
        if ($file->CanViewType != $CanViewType) {
            $file->CanViewType = $CanViewType;
    ...
    public function RenderMe($Layout = Download::class): DBHTMLText
    {
    ...
    protected function onAfterWrite()
    {
        ...
        if ($this->PreviewThumbnailID == 0 && Config::inst()->get("DownloadModuleConfig")["PreviewImageEnabled"]) {
            ...
            try {
                if (stripos($this->File()->FileName, ".pdf") !== false) {
